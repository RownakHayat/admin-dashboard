...
import { useGetCategoryWiseRunningEventHomeQuery } from "@/store/features/home";
...
const CatagoryWiseSingleEvent = () => {
  ...
  const listIds = searchParams.get("listIds"); // Extract the listIds query parameter

    const {
      data: CatagoryWiseSingleEventData,
      isLoading,
      isError,
      refetch,
    } = useGetCategoryWiseRunningEventHomeQuery({ id,  listIds} )
  ...
  return (
    <div className="custom_container py-[50px]">
      <ReactTable dataSource={CatagoryWiseSingleEventData?.data} columns={columns} isLoading={isLoading} />
    </div>
  )
}
