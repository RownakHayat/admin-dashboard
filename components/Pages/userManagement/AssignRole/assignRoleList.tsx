"use client";

import ReactTable from "@/components/common/ReactTable/ReactTable";
import SwitchButton from "@/components/common/ReactTable/switchButton/SwitchButton";
import Search from "@/components/common/Search/Search";
import { useFormSetting } from "@/components/common/hooks/useFormSetting";
import { IndexSerial } from "@/components/common/utils";
import { Icons } from "@/components/icons";
import { Button } from "@/components/ui/button";
import {
  useGetUsersAssignRollPaginationQuery,
  useUsersRollDeleteMutation
} from "@/store/features/UserManagement/Roll";
import { useChangeUsersStatusMutation } from "@/store/features/UserManagement/Users";
import { createColumnHelper } from "@tanstack/react-table";
import Link from "next/link";
import { useMemo } from "react";

const columnHelper = createColumnHelper<any>();

const AssignRollComponent = () => {
  const { params, editData, filterSearchText, searchField } = useFormSetting();

  const paramsValue = {
    ...params,
    searchData: `${[[`${filterSearchText && filterSearchText}`]]}`,
  };

  const {
    data: listQuery,
    refetch,
    isLoading,
  } = useGetUsersAssignRollPaginationQuery(paramsValue);
  const [ChangeStatus] = useChangeUsersStatusMutation();
  const [deleteUserRoll] = useUsersRollDeleteMutation();

  const handleDelete = async (id: number) => {
    alert(id);
    try {
      await deleteUserRoll(id).unwrap();
      refetch();
    } catch (error) {
    }
  };

  const columns: any = useMemo(
    () => [
      columnHelper.accessor((tableField) => tableField.id, {
        id: "id",
        header: "SL",
        cell: (props: any) => {
          const sl = IndexSerial(
            params?.page,
            params.limit,
            props.row.index,
            listQuery?.pagination?.total
          );
          return sl;
        },
      }),
      columnHelper.accessor((tableField) => tableField?.name, {
        id: "name",
        header: "Name",
      }),
      columnHelper.accessor((tableField) => tableField?.role?.name, {
        id: "role",
        header: "Roll",
      }),
      columnHelper.accessor(() => "", {
        id: "action",
        header: "Action",
        cell: ({ row }: any) => {
          const viewData = row?.original;
          return (
            <div className=" flex justify-left space-x-3">
              <SwitchButton
                updateAPI={ChangeStatus}
                data={{
                  ...row?.original,
                  id: row?.original.id,
                }}
              />

              <span className="cursor-pointer">
                <Link
                  href={`/admin/user-management/assign-role/assign-role-create/${viewData?.id}/edit`}
                >
                  <Icons.edit onClick={() => editData(viewData)} />
                </Link>
              </span>
              <span
                className="cursor-pointer"
                onClick={() => handleDelete(row.original.id)}
              >
                <Icons.delete />
              </span>

            </div>
          );
        },
      }),
    ],
    [params, listQuery]
  );

  return (
    <div>
      <div className="grid grid-cols-12 items-center ">
        <div className="col-span-12 md:col-span-10">
          <Search />
        </div>
        <div className="col-span-12 md:col-span-2 bg-[#F5F3Fa] py-5">
          <Link href="/admin/user-management/assign-role/assign-role-create">
            <Button className="ml-5 bg-headerbg text-primary border-primary border flex items-center gap-2 hover:text-primary hover:border-primary hover:bg-headerbg">
              <Icons.plus size={15} />
              Create Assign Roll
            </Button>
          </Link>
        </div>
      </div>
      <ReactTable
        dataSource={listQuery}
        columns={columns}
        isLoading={isLoading}
      />
    </div>
  );
};

export default AssignRollComponent;
