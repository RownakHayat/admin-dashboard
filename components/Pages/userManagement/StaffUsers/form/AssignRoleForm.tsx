...
import { z } from "zod";
import { StaffUsersRollSchema } from "../schemas/staffUsersSchema";

interface StaffRollDialogProps {
  open: boolean;
  onClose: () => void;
  onSave: (values: z.infer<typeof StaffUsersRollSchema>) => void;
  initialValues?: any;
}

const CreateAssignStaffRoleForm: React.FC<StaffRollDialogProps> = ({
  open,
  onClose,
  onSave,
  initialValues,
}) => {
  const { data: userRoleData } = useGetAllActiveUsersRollListQuery();
  const [createStaffUsersRole] = useUserStaffRoleAssignPermissionMutation();
  const router = useRouter()
  const number: number = initialValues?.id;
  const form = useForm<z.infer<typeof StaffUsersRollSchema>>({
    resolver: zodResolver(StaffUsersRollSchema),
    defaultValues: {
  ...
  const onSubmitHandler: SubmitHandler<
    z.infer<typeof StaffUsersRollSchema>
  > = async (values) => {
      ...
      if (err?.data?.errors) {
        err?.data?.errors?.forEach(
          ({ field, message }: { field: string; message: string }) =>
            form.setError(field as keyof z.infer<typeof StaffUsersRollSchema>, {
              type: "custom",
              message,
            })
        );
      ...
      <DialogContent>
        ...
        <FormProvider {...form}>
          <form onSubmit={form.handleSubmit(onSubmitHandler)}>
            ...
            <FormInput name="name" label="Staff Name" disabled />

            <FormAutoComplete
              name="roles_id"
              data={listArrayDaynamicModify(userRoleData?.data, "roles", "name")}
...
export default CreateAssignStaffRoleForm;
