...
import { useCreateFinancialYearMutation, useFinancialYearUpdateMutation } from '@/store/features/configuration/financialYear';
import { zodResolver } from '@hookform/resolvers/zod';
import React, { useEffect } from 'react';
import { SubmitHandler, useForm } from 'react-hook-form';
import { z } from 'zod';
import { finacialYearsSchema } from '../schemas/finacialYearsSchema';
import { useRouter } from 'next/navigation';

type FinacialYearsSFormProps = {
    refetch: () => void;
};

const FinacialYearsSForm: React.FC<FinacialYearsSFormProps> = ({ refetch }) => {
    const { showData, editMode, closeFormToggle } = useFormSetting();
    const { ToastSuccess, ToastError } = useToast();
    const [createUpazila] = useCreateFinancialYearMutation();
    const [updateUpazila] = useFinancialYearUpdateMutation();
    const router = useRouter();

    const form = useForm<z.infer<typeof finacialYearsSchema>>({
        resolver: zodResolver(finacialYearsSchema),
        defaultValues: {
            name: '',
        },
    });
    ...
    const onSubmit: SubmitHandler<z.infer<typeof finacialYearsSchema>> = async (values) => {
        try {
            ...
            err?.data?.errors?.forEach(({ field, message }: { field: string; message: string }) =>
                form.setError(field as keyof z.infer<typeof finacialYearsSchema>, {
                    type: 'custom',
                    message,
                })
            );
    ...
    return (
        <div className="w-full bg-[#f5f3fa] rounded my-3">
            <div className="bg-headerbg p-5 mb-3">
                <p className="text-2xl">{editMode ? 'Update' : 'Create'} Financial Year</p>
            </div>
...
export default FinacialYearsSForm;
