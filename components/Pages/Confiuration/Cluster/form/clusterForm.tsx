...
import { useClusterUpdateMutation, useCreateClusterMutation } from '@/store/features/configuration/cluster';
import { zodResolver } from '@hookform/resolvers/zod';
import React, { useEffect } from 'react';
import { SubmitHandler, useForm } from 'react-hook-form';
import { z } from 'zod';
import { clusterSchema } from '../schemas/clusterSchema';
import { useRouter } from 'next/navigation';

type ClusterFormProps = {
    refetch: () => void;
};
const ClusterForm: React.FC<ClusterFormProps> = ({ refetch }) => {
    const { showData, editMode, closeFormToggle } = useFormSetting();
    const { ToastSuccess, ToastError } = useToast();
    const [createUpazila] = useCreateClusterMutation();
    const [updateUpazila] = useClusterUpdateMutation();
 ...
 const onCancelClick = () => {
    ...
    const onSubmit: SubmitHandler<z.infer<typeof clusterSchema>> = async (values) => {
        try {
            ...
            err?.data?.errors?.forEach(({ field, message }: { field: string; message: string }) =>
                form.setError(field as keyof z.infer<typeof clusterSchema>, {
                    type: 'custom',
                    message,
                })
            );
    ...
    return (
        <div className="w-full bg-[#f5f3fa] rounded my-3">
            <div className="bg-headerbg p-5 mb-3">
                <p className="text-2xl">{editMode ? 'Update' : 'Create'} Cluster</p>
            </div>
