...
import { z } from "zod";
import { SMECategorySchemas } from "../schemas/smeCategorySchemas";
import { useCreateSmeCategoryMutation, useSmeCategoryUpdateMutation } from "@/store/features/configuration/smeCategory";
import { useRouter } from "next/navigation";

type SMECategoryTypeProps = {
  refetch: () => void;
};

const SmeCategoryForm: React.FC<SMECategoryTypeProps> = ({ refetch }) => {
  const { showData, editMode, closeFormToggle } = useFormSetting();
  const { ToastSuccess, ToastError } = useToast();
  const [createSMECategory] = useCreateSmeCategoryMutation();
  const [updateSMECategory] = useSmeCategoryUpdateMutation();
  const router = useRouter();

  const form = useForm<z.infer<typeof SMECategorySchemas>>({
    resolver: zodResolver(SMECategorySchemas),
    defaultValues: {
      name: "",
    },
  });
  ...
  const onSubmit: SubmitHandler<z.infer<typeof SMECategorySchemas>> = async (
    values
  ) => {
    try {
      const mutationFn = editMode ? updateSMECategory : createSMECategory;
      ...
      err?.data?.errors?.forEach(
        ({ field, message }: { field: string; message: string }) =>
          form.setError(field as keyof z.infer<typeof SMECategorySchemas>, {
            type: "custom",
            message,
          })
      );
  ...
  return (
    <div className="w-full bg-[#f5f3fa] rounded my-3">
      <div className="bg-headerbg p-5 mb-3">
        <p className="text-2xl">{editMode ? "Update" : "Create"} SME Category</p>
      </div>
        ...
        <FormContainer form={form} onSubmit={form.handleSubmit(onSubmit)}>
          <div className="grid grid-cols-12 gap-2 mt-0 items-center">
            <div className="col-span-12 md:col-span-6">
              <FormInput
                name="name"
                placeholder="Enter SME Category name"
                label="SME Category Name"
                remark={true}
              />
            </div>
...
export default SmeCategoryForm;
