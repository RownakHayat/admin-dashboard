...
import { useCreateOrganizerMutation, useOrganizerSetupUpdateMutation } from '@/store/features/configuration/organizer';
import { zodResolver } from '@hookform/resolvers/zod';
import React, { useEffect } from 'react';
import { SubmitHandler, useForm } from 'react-hook-form';
import { z } from 'zod';
import { organizerFormSchema } from '../schemas/organizerFormSchema';
...
const OrganizerForm: React.FC<OrganizerFormProps> = ({ refetch }) => {
    const { showData, editMode, closeFormToggle } = useFormSetting();
    const { ToastSuccess, ToastError } = useToast();
    const [createUpazila] = useCreateOrganizerMutation();
    const [updateUpazila] = useOrganizerSetupUpdateMutation();
    ...
    const onSubmit: SubmitHandler<z.infer<typeof organizerFormSchema>> = async (values) => {
        try {
            ...
            err?.data?.errors?.forEach(({ field, message }: { field: string; message: string }) =>
                form.setError(field as keyof z.infer<typeof organizerFormSchema>, {
                    type: 'custom',
                    message,
                })
            );
    ...
    return (
        <div className="w-full bg-[#f5f3fa] rounded my-3">
            <div className="bg-headerbg p-5 mb-3">
                <p className="text-2xl">{editMode ? 'Update' : 'Create'} Organizer</p>
            </div>
