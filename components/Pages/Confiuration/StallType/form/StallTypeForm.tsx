...
import { z } from "zod";
import { StallTypeSchemas } from "../schemas/StallTypeSchemas";
import { useCreateStallTypeMutation, useStallTypeUpdateMutation } from "@/store/features/configuration/stallType";
...
const StallTypeForm: React.FC<UserTypeFormProps> = ({ refetch }) => {
  const { showData, editMode, closeFormToggle } = useFormSetting();
  const { ToastSuccess, ToastError } = useToast();
  const [createStallType] = useCreateStallTypeMutation();
  const [updateStallType] = useStallTypeUpdateMutation();
  const router = useRouter();
  const form = useForm<z.infer<typeof StallTypeSchemas>>({
    resolver: zodResolver(StallTypeSchemas),
    defaultValues: {
      name: "",
      name_bn: "",
      stall_fare: "",
  ...
  const onSubmit: SubmitHandler<z.infer<typeof StallTypeSchemas>> = async (
    values
  ) => {
    try {
      const mutationFn = editMode ? updateStallType : createStallType;
      ...
      err?.data?.errors?.forEach(
        ({ field, message }: { field: string; message: string }) =>
          form.setError(field as keyof z.infer<typeof StallTypeSchemas>, {
            type: "custom",
            message,
          })
      );
  ...
  useEffect(() => {
    form.reset({
      ...
      stall_fare: showData?.stall_fare?.toString() || "",
    });
  ...
  return (
    <div className="w-full bg-[#f5f3fa] rounded my-3">
      <div className="bg-headerbg p-5 mb-3">
        <p className="text-2xl">{editMode ? "Update" : "Create"} Stall Type</p>
      </div>
        ...
        <FormContainer form={form} onSubmit={form.handleSubmit(onSubmit)}>
          <div className="grid grid-cols-12 gap-2 mt-0 items-center">
            ...
            <div className="col-span-12 md:col-span-4">
              <FormInput
                name="name_bn"
                placeholder="Enter Stall Type (Bangla)"
                label="Stall Type Name (Bangla)"
                bengaliAllow={true}
              />
            </div>
            <div className="col-span-12 md:col-span-4">
