...
import { useCreateDesignationMutation, useDesignationUpdateMutation } from '@/store/features/configuration/designation';
...
const DesignationForm: React.FC<UserTypeFormProps> = ({ refetch }) => {
    const { showData, editMode, closeFormToggle } = useFormSetting();
    const { ToastSuccess, ToastError } = useToast();
    const [createUserType] = useCreateDesignationMutation();
    ...
    return (
        <div className="w-full bg-[#f5f3fa] rounded my-3">
            <div className="bg-headerbg p-5 mb-3">
                <p className="text-2xl">{editMode ? 'Update' : 'Create'} Designation</p>
            </div>
