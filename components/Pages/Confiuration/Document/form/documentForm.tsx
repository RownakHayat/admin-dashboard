...
import { useCreateDocumentMutation, useUpdateDocumentMutation } from '@/store/features/configuration/document';
...
const DocumentForm: React.FC<UserTypeFormProps> = ({ refetch }) => {
    ...
    const onSubmit: SubmitHandler<z.infer<typeof userTypeSchema>> = async (values) => {
        ...
        try {
            const mutationFn = editMode ? updateDocument : createDocument;
    ...
    return (
        <div className="w-full bg-[#f5f3fa] rounded my-3">
            <div className="bg-headerbg p-5 mb-3">
                <p className="text-2xl">{editMode ? 'Update' : 'Create'} Document</p>
            </div>
