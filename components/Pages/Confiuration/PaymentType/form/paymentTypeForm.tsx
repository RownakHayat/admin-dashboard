...
import { z } from 'zod';
import { PaymentTypeSchema } from '../schemas/paymentTypeSchemas';
...
const PaymentTypeForm: React.FC<OrganizerFormProps> = ({ refetch }) => {
    ...
    const onSubmit: SubmitHandler<z.infer<typeof PaymentTypeSchema>> = async (values) => {
        try {
            ...
            err?.data?.errors?.forEach(({ field, message }: { field: string; message: string }) =>
                form.setError(field as keyof z.infer<typeof PaymentTypeSchema>, {
                    type: 'custom',
                    message,
                })
            );
    ...
    return (
        <div className="w-full bg-[#f5f3fa] rounded my-3">
            <div className="bg-headerbg p-5 mb-3">
                <p className="text-2xl">{editMode ? 'Update' : 'Create'} Payment Type</p>
            </div>
