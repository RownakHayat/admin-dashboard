...
import { useChatTropicsUpdateMutation, useCreateChatTropicsMutation } from '@/store/features/configuration/chatTropics';
import { zodResolver } from '@hookform/resolvers/zod';
import React, { useEffect } from 'react';
import { SubmitHandler, useForm } from 'react-hook-form';
import { z } from 'zod';
import { chatTropicsSchema } from '../schemas/chatTropicsSchema';
import { useRouter } from 'next/navigation';

type ChatTropicsFormProps = {
    refetch: () => void;
};

const ChatTropicsForm: React.FC<ChatTropicsFormProps> = ({ refetch }) => {
    const { showData, editMode, closeFormToggle } = useFormSetting();
    const { ToastSuccess, ToastError } = useToast();
    const [createchatTropics] = useCreateChatTropicsMutation();
    const [updatechatTropics] = useChatTropicsUpdateMutation();
    const router = useRouter();

    const form = useForm<z.infer<typeof chatTropicsSchema>>({
        resolver: zodResolver(chatTropicsSchema),
        defaultValues: {
            name: '',
        },
    });
    ...
    const onSubmit: SubmitHandler<z.infer<typeof chatTropicsSchema>> = async (values) => {
        try {
            const mutationFn = editMode ? updatechatTropics : createchatTropics;
            ...
            err?.data?.errors?.forEach(({ field, message }: { field: string; message: string }) =>
                form.setError(field as keyof z.infer<typeof chatTropicsSchema>, {
                    type: 'custom',
                    message,
                })
            );
    ...
    return (
        <div className="w-full bg-[#f5f3fa] rounded my-3">
            <div className="bg-headerbg p-5 mb-3">
                <p className="text-2xl">{editMode ? 'Update' : 'Create'} Chat Tropics</p>
            </div>
                ...
                <FormContainer form={form} onSubmit={form.handleSubmit(onSubmit)}>
                    <div className="grid grid-cols-12 gap-2 mt-0 items-center">
                        <div className="col-span-12 md:col-span-6">
                            <FormInput
                                name="name"
                                placeholder="Enter Chat Tropics"
                                label="Name"
                                remark={true}
                            />
                        </div>
...
export default ChatTropicsForm;
