...
import { useCreateEducationalQualificationMutation, useUpdateEducationalQualificationMutation } from '@/store/features/configuration/educationalQualification';
...
const EducationalQualificationForm: React.FC<UserTypeFormProps> = ({ refetch }) => {
    const { showData, editMode, closeFormToggle } = useFormSetting();
    const { ToastSuccess, ToastError } = useToast();
    const router = useRouter();
    const [createEducationalQualification] = useCreateEducationalQualificationMutation();
    const [updateEducationalQualification] = useUpdateEducationalQualificationMutation();
    ...
    const onSubmit: SubmitHandler<z.infer<typeof userTypeSchema>> = async (values) => {
        ...
        try {
            const mutationFn = editMode ? updateEducationalQualification : createEducationalQualification;
    ...
    return (
        <div className="w-full bg-[#f5f3fa] rounded my-3">
            <div className="bg-headerbg p-5 mb-3">
                <p className="text-2xl">{editMode ? 'Update' : 'Create'} Educational Qualification</p>
            </div>
