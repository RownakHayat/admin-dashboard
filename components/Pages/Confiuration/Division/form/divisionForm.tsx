...
import { useCreateDivisionMutation, useDivisionUpdateMutation } from '@/store/features/configuration/division';
...
const DivisionForm: React.FC<UserTypeFormProps> = ({ refetch }) => {
    ...
    const onSubmit: SubmitHandler<z.infer<typeof divisionSchema>> = async (values) => {
        try {
            const mutationFn = editMode ? updateDivision : createDivision;
            ...
            err?.data?.errors?.forEach(({ field, message }: { field: string; message: string }) =>
                form.setError(field as keyof z.infer<typeof divisionSchema>, {
                    type: 'custom',
                    message,
                })
            );
    ...
    return (
        <div className="w-full bg-[#f5f3fa] rounded my-3">
            <div className="bg-headerbg p-5 mb-3">
                <p className="text-2xl">{editMode ? 'Update' : 'Create'} Division</p>
            </div>
