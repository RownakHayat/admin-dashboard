...
import { useCreateOccupationTypeMutation, useOccupationTypeUpdateMutation } from '@/store/features/configuration/occupationType';
...
type OccupationTypeFormProps = {
    refetch: () => void;
};

const OccupationTypeForm: React.FC<OccupationTypeFormProps> = ({ refetch }) => {
    const { showData, editMode, closeFormToggle } = useFormSetting();
    const { ToastSuccess, ToastError } = useToast();
    const [createUpazila] = useCreateOccupationTypeMutation();
    const [updateUpazila] = useOccupationTypeUpdateMutation();
    ...
    return (
        <div className="w-full bg-[#f5f3fa] rounded my-3">
            <div className="bg-headerbg p-5 mb-3">
                <p className="text-2xl">{editMode ? 'Update' : 'Create'} Occupation Type</p>
            </div>
...
export default OccupationTypeForm;
