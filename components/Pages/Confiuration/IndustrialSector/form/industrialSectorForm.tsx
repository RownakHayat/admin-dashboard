...
import { useCreateIndustrialSectorMutation, useIndustrialSectorUpdateMutation } from '@/store/features/configuration/industrialSector';
import { zodResolver } from '@hookform/resolvers/zod';
import React, { useEffect } from 'react';
import { SubmitHandler, useForm } from 'react-hook-form';
import { z } from 'zod';
import { industrialSectorSchema } from '../schemas/industrialSectorSchema';
import { useRouter } from 'next/navigation';

type IndustrialSectorFormProps = {
    refetch: () => void;
};

const IndustrialSectorForm: React.FC<IndustrialSectorFormProps> = ({ refetch }) => {
    const { showData, editMode, closeFormToggle } = useFormSetting();
    const { ToastSuccess, ToastError } = useToast();
    const router = useRouter();
    const [createUpazila] = useCreateIndustrialSectorMutation();
    const [updateUpazila] = useIndustrialSectorUpdateMutation();

    const form = useForm<z.infer<typeof industrialSectorSchema>>({
        resolver: zodResolver(industrialSectorSchema),
        defaultValues: {
            name: '',
        },
    });
    ...
    const onSubmit: SubmitHandler<z.infer<typeof industrialSectorSchema>> = async (values) => {
        try {
            ...
            err?.data?.errors?.forEach(({ field, message }: { field: string; message: string }) =>
                form.setError(field as keyof z.infer<typeof industrialSectorSchema>, {
                    type: 'custom',
                    message,
                })
            );
    ...
    return (
        <div className="w-full bg-[#f5f3fa] rounded my-3">
            <div className="bg-headerbg p-5 mb-3">
                <p className="text-2xl">{editMode ? 'Update' : 'Create'} Industrial Sector</p>
            </div>
                ...
                <FormContainer form={form} onSubmit={form.handleSubmit(onSubmit)}>
                    <div className="grid grid-cols-12 gap-2 mt-0 items-center">
                        <div className="col-span-12 md:col-span-6">
                            <FormInput
                                name="name"
                                placeholder="Enter Industrial Sector"
                                label="Name"
                                remark={true}
                            />
                        </div>
...
export default IndustrialSectorForm;
