...
const EventDetailsPageView = () => {
    ...
    const {
        ...
        refetch
    } = useGetSingleEventDetailsQuery({ id: id }, { refetchOnMountOrArgChange: true });
    ...
    const handleComplete = async (id: number | null) => {
        try {
            ...
            Swal.fire(
                'Error!',
                error?.data?.message || 'An error occurred while completing the event.',
                'error'
            );
        }
    };
          ...
          <div className="flex flex-wrap">
            ...
            <ul className="ml-2 list-inside">
              {singleEventViewData?.data?.event_attachments?.map(
                ...
                {/* Back Button */}
                <div className="col-span-12 md:col-span-6 text-end space-x-2">
                    <Button
                        onClick={() => router.back()}
                        className="bg-gray-500 rounded-lg px-3 py-2 text-white"
                    >
                        Back
                    </Button>

                    {!['applied', 'removed_from_selected', 'selected'].includes(
                        singleEventViewData?.data?.event_application?.[0]?.application_status
                    ) && (
                            <Link href={`/admin/events/new-event-apply/${singleEventViewData?.data?.id}/apply-event`}>
                                <Button className="bg-[#1ca153] rounded-lg px-3 py-2 text-white ml-2">Apply</Button>
                            </Link>
                        )}
                </div>
